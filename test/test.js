...
const totalCostFn = require('../totalCost');
...
describe(`Test run for ${urlUnderTest}`, () => {
    it('Asserts the total from 10-12-2017 for 20 days is $2.45', done => {
      request.get(
        ...
        expect(body.totalCost).to.equal('$2.45');
        expect(totalCostFn('10-12-2017', 20)).to.equal(body.totalCost);
        done();
            }
        );
    });
it('Asserts the total from 01-01-2018 for 1 day is $0.05', done => {
    request.get(
        ...
        expect(totalCostFn('01-01-2018', 1)).to.equal(body.totalCost);
        done();
            }
        );
    });
...
it('Asserts the total from 01-29-2018 for 1 day is $0.25', done => {
    request.get(
        ...
        expect(totalCostFn('01-29-2018', 1)).to.equal(body.totalCost);
        done();
            }
        );
    });
